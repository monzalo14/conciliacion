...
# Lista de razones y sus expresiones conocidas
razones <- list(
...
top_dems <- dems %>% 
        mutate(nombre = limpia_razones(razones, nombre)) %>%
        filter(!is.na(nombre)) %>%
        dplyr::count(nombre, sort = T) %>%
        filter(n > 10) %>% 
        .$nombre

save(top_dems, file ='../data/top_demandados.RData')
